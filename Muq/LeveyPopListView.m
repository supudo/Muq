...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions {
    ...
    if (self = [super initWithFrame:rect]) {
        ...
        float tblY = POPLISTVIEW_HEADER_HEIGHT + (rect.size.height - ([_options count] * 44 + POPLISTVIEW_HEADER_HEIGHT + RADIUS)) / 2;
        float tblHeight = ([_options count] * 44 + RADIUS);
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(POPLISTVIEW_SCREENINSET,
                                                                   tblY,//POPLISTVIEW_SCREENINSET + POPLISTVIEW_HEADER_HEIGHT,
...
- (void)drawRect:(CGRect)rect {
    float bgHeight = ([_options count] * 44 + POPLISTVIEW_HEADER_HEIGHT + RADIUS);
    if (bgHeight > rect.size.height)
        bgHeight = rect.size.height -  44;
    
    float bgY = (rect.size.height - bgHeight) / 2;
    CGRect bgRect = CGRectMake(rect.origin.x, bgY, (rect.size.width - (POPLISTVIEW_SCREENINSET * 2)), bgHeight);
    
    float titleY = bgY + 15;
    CGRect titleRect = CGRectMake(POPLISTVIEW_SCREENINSET + 10, titleY, rect.size.width - 2 * (POPLISTVIEW_SCREENINSET + 10), 30);
    
    float sepY = POPLISTVIEW_SCREENINSET + POPLISTVIEW_HEADER_HEIGHT - 2;
    sepY = bgY + POPLISTVIEW_HEADER_HEIGHT - 2;
    CGRect separatorRect = CGRectMake(POPLISTVIEW_SCREENINSET, sepY, rect.size.width - 2 * POPLISTVIEW_SCREENINSET, 2);

    //CGRect bgRect = CGRectInset(rect, POPLISTVIEW_SCREENINSET, POPLISTVIEW_SCREENINSET);
    //CGRect titleRect = CGRectMake(POPLISTVIEW_SCREENINSET + 10, POPLISTVIEW_SCREENINSET + 10 + 5, rect.size.width -  2 * (POPLISTVIEW_SCREENINSET + 10), 30);
    ...
    [[UIColor colorWithRed:100 / 255.0 green:176 / 255.0 blue:252 / 255.0 alpha:1.0] setFill];
    [_title drawInRect:titleRect withFont:[UIFont fontWithName:[MQSettings sharedInstance].fontName size:16.0]];
    //[_title drawInRect:titleRect withAttributes:@{ NSFontAttributeName : [UIFont fontWithName:[PFSettings sharedInstance].fontName size:16.0] }];
